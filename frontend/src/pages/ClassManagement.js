...
function ClassManagement({ user, onLogout, onClassSelect }) {
  ...
  const loadClasses = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authenticatedFetch('https://52.23.173.216:5050/api/teacher/classes');
      if (res.ok) {
  ...
  const handleEditClass = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://52.23.173.216:5050/api/teacher/classes/${editingClass.id}`, {
        method: 'PUT',
  ...
  const handleDeleteClass = async (classId) => {
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://52.23.173.216:5050/api/teacher/classes/${classId}`, {
        method: 'DELETE'
      });
