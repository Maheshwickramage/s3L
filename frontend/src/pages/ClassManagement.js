...
function ClassManagement({ user, onLogout, onClassSelect }) {
  ...
  const loadClasses = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authenticatedFetch('https://s3-alb-1110116241.us-east-1.elb.amazonaws.com/api/teacher/classes');
      if (res.ok) {
  ...
  const handleEditClass = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://s3-alb-1110116241.us-east-1.elb.amazonaws.com/api/teacher/classes/${editingClass.id}`, {
        method: 'PUT',
  ...
  const handleDeleteClass = async (classId) => {
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://s3-alb-1110116241.us-east-1.elb.amazonaws.com/api/teacher/classes/${classId}`, {
        method: 'DELETE'
      });
