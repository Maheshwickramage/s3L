...
function ClassManagement({ user, onLogout, onClassSelect }) {
  ...
  const loadClasses = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authenticatedFetch('https://api.s3learn.com/api/teacher/classes');
      if (res.ok) {
  ...
  const handleEditClass = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://api.s3learn.com/api/teacher/classes/${editingClass.id}`, {
        method: 'PUT',
  ...
  const handleDeleteClass = async (classId) => {
    setLoading(true);
    try {
      const res = await authenticatedFetch(`https://api.s3learn.com/api/teacher/classes/${classId}`, {
        method: 'DELETE'
      });
